...
import validateMealInput from "../../validation/meal.js";
...
awsBucketToken,
    awsRegion,
UploadFileUrlLink,
SPOONACULAR_API_KEY} from '../../config/keys.js';
...
router.get("/:spoonacularId", async (req, res) => {
    try {
        const spoonacularResponseJson = await fetch(`https://api.spoonacular.com/recipes/${req.params.spoonacularId}/information?apiKey=${SPOONACULAR_API_KEY}`);
        const spoonacularResponse = await spoonacularResponseJson.json();

        res.status(200).json(spoonacularResponse);
    } catch (e){
